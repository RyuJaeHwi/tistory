...
인터페이스의 기본 구조는 다음과 같다.`interface 인터페이스_이름 {
속성: 타입;
}`  
...
콜론(:) 앞에 물음표(?) 를 붙이면 된다.`interface 인터페이스_이름 {
속성?: 타입;
}`  
실제 예제 코드를 통해 알아보자.
...
readonly를 속성 이름 앞에 붙이면 적용된다.`interface 인터페이스_이름 {
readonly 속성: 타입;
}`  
...
ex)`let arr: ReadonlyArray<number> = [1,2,3]; // 읽기 전용 배열
...
인터페이스는 **extends**나 **implement** 키워드 등을 사용하여 단순 객체 타입 표현을 넘어 다양하게 활용이 가능하다.
